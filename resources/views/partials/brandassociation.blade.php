<div class="brand-association">
    <div class="row">
        <div class="brand-logos col-md-10 px-0">
            <div class="-text">Shop your favourite brands:</div>
            <a href="#"><img class="brand-logo -cb2" src="{{ asset('/images/brand-images/cb2.png') }}" alt="CB2 brand"></a>
            <a href="#"><img class="brand-logo -crate" src="{{ asset('/images/brand-images/crate-barrel.png') }}" alt="Crate and barrel brand"></a>
            <a href="#"><img class="brand-logo -pier1" src="{{ asset('/images/brand-images/pier1.png') }}" alt="Pier1 brand"></a>
            <a href="#"><img class="brand-logo -westelm" src="{{ asset('/images/brand-images/westelm.png') }}" alt="WestElm brand"></a>
            <a href="#"><img class="brand-logo -worldmarket" src="{{ asset('/images/brand-images/worldmarket.png') }}" alt="World Market brand"></a>

            <img class="brand-logo more-soon" src="{{ asset('/images/brand-images/more.png') }}" alt="and more coming soon">
        </div>
        <div class="socials col-md-2 px-0">
            <div class="-text">Get in touch</div>
            <div class="-icons">
                <a target="_blank" href="//www.facebook.com/shoplazysuzy" class="-icon"><i class="fab fa-facebook-f -cicon"></i></a>
                <i class="fab d-none fa-instagram -icon"></i>
                <i class="fab d-none fa-pinterest -icon"></i>
            </div>
        </div>
    </div>
</div>